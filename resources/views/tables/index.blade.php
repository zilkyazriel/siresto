<x-app-layout>
    <div class="mx-auto w-full max-w-[1440px]">
        ...
        <div class="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
            <div>
                ...
                <p class="text-sm text-slate-500 dark:text-slate-400">Kelola nomor & kapasitas meja. Status meja tampil otomatis di Denah.</p>
            </div>
            <div class="flex items-center gap-2">
                <a href="{{ route('tables.denah') }}"
                    class="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-5 py-3 text-sm font-semibold text-slate-700 shadow-sm transition-all hover:border-orange-300 hover:text-orange-600 hover:shadow-md active:scale-95 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200">
        ...
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            @forelse ($tables as $table)
                <div class="group relative flex flex-col rounded-2xl border border-transparent bg-white p-5 shadow-sm transition-shadow duration-300 hover:border-slate-200 hover:shadow-md dark:bg-slate-800 dark:hover:border-slate-700">
                    ...
                    <a href="{{ route('tables.denah') }}" class="mt-auto inline-flex items-center gap-1 text-xs font-medium text-slate-400 transition-colors hover:text-orange-500">
                        <span class="material-symbols-outlined text-[14px]">grid_view</span>
                        Lihat status di Denah
                    </a>
                </div>
            @empty
    ...
    <div id="tableModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/50 p-4">
        <div class="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl dark:bg-slate-800">
            ...
            <form id="tableForm" method="POST" action="{{ route('tables.store') }}" class="space-y-4">
                ...
                <!-- Status disembunyikan (Opsi B: status otomatis dari pesanan, tampil di Denah) -->
                <input type="hidden" name="status" id="fStatus" value="tersedia">
                <div>
    ...
    <script>
        ...
        function openEdit(btn) {
            ...
            document.getElementById('fStatus').value = btn.dataset.status || 'tersedia';
            showModal();
        }
